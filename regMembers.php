...
if (isset($_SESSION['usermail']))
{
    ...
    if ($do == 'Manage')
    {
        ...
        $db->query("SELECT * FROM users_registered $query");
...
<form class="login-form"  enctype="multipart/form-data" action="?do=Update" method="POST">
    ...
    else if ($do == 'Update')
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST')
        {
            ...
            $db->query("UPDATE users_registered SET firstname = :firstname, lastname = :lastname, regmail = :regmail, reginfo = :reginfo WHERE regId = :regId ");

            $db->bind(":firstname", $firstname);
            $db->bind(":lastname", $lastname);
            $db->bind(":regmail", $regmail);
            $db->bind(":reginfo", $reginfo);
            $db->bind(":regUserId", $regUserId);
            $db->execute();
