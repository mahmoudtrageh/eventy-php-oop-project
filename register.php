...
move_uploaded_file($filetmp, '../assets/images/uploads/'.$r.$d.$filename);
    ...
    if($password == $cpassword) {
        ...
        $stmt1 = $con->prepare("SELECT usermail FROM users WHERE usermail = ?");
                $stmt1 ->execute(array($usermail));
                $user = $stmt1->fetchAll(PDO::FETCH_ASSOC);
    
    if(empty($user)) {
       

        $ins = $con->prepare(" INSERT INTO users (usermail, password, cpassword, userpic, firstname, lastname) VALUES ('$usermail', '$password', '$cpassword', '$finalDes', '$firstname', '$lastname')");
        $ins->execute(array($usermail, $password, $cpassword, $finalDes, $firstname, $lastname));
